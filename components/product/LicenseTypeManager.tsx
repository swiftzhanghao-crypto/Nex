...
const FIELD_GROUPS: FieldGroup[] = [
  { title: '基础信息', fields: [
    { key: 'pricingQty', name: '计价数量', dt: 'configurable', type: '数字', desc: '可配置，部分视为1' },
  ]},
  { title: '日期与期限', fields: [
    { key: 'startDateCalc', name: '授权开始日期计算', dt: 'fixed', type: '枚举', desc: '支持选项自定义', enumDefs: ['发货后5个自然日开始计算', '按上一授权截止日期开始计算', '按申请日期开始', '权益生效后计算'] },
    { key: 'prevEndDate', name: '上一授权截止日期', dt: 'conditional', type: '日期', desc: '和授权开始计算日期联动', cond: "授权开始日期计算='从上一期截止日期开始'" },
    { key: 'applyStart', name: '申请服务开始时间', dt: 'conditional', type: '日期', desc: '和授权开始计算日期联动', cond: "授权开始日期计算='按已申请日期填写'" },
    { key: 'expectEnd', name: '预计服务结束时间', dt: 'conditional', type: '日期', desc: '和授权开始计算日期联动' },
    { key: 'orderByPeriod', name: '按期限下单/按截止时间下单', dt: 'fixed', type: '枚举', desc: '支持选项自定义', enumDefs: ['按期限下单', '按截止时间下单'] },
    { key: 'authStartDate', name: '授权开始日期', dt: 'configurable', type: '日期', desc: '可配置是否展示' },
    { key: 'authEndDate', name: '授权截止日期', dt: 'conditional', type: '日期', desc: '和【按期限/截止时间下单】联动' },
  ]},
  { title: '升级保障', fields: [
    { key: 'upgradePeriod', name: '升级保障期限', dt: 'configurable', type: '数字', desc: '默认值=授权期限，允许编辑' },
    { key: 'upgradeStart', name: '升级保障开始日期', dt: 'configurable', type: '日期', desc: '可配置' },
    { key: 'upgradeEnd', name: '升级保障截止日期', dt: 'configurable', type: '日期', desc: '可配置' },
  ]},
  { title: '交付与服务', fields: [
    { key: 'delivery', name: '交付方式', dt: 'configurable', type: '枚举', desc: '选项：一次性交付/分期交付' },
    { key: 'expiredQty', name: '到期后授权数量', dt: 'configurable', type: '数字', desc: '可配置是否展示' },
    { key: 'serialDelay', name: '支持序列号失效延长时间', dt: 'configurable', type: '数字', desc: '可配置/条件展示' },
  ]},
  { title: '授权范围与主体', fields: [
    { key: 'licensee', name: '被授权方', dt: 'configurable', type: '文本', desc: '支持置入默认值' },
    { key: 'authScope', name: '授权范围', dt: 'configurable', type: '文本', desc: '支持默认值，校验是否含xx' },
    { key: 'siteUsers', name: '端年场地授权覆盖的用户数量', dt: 'configurable', type: '数字', desc: '可配置' },
    { key: 'subUnit', name: '按下级单位分开提供授权', dt: 'conditional', type: '枚举', desc: '仅新购时展示', cond: '仅新购' },
  ]},
];
...
function initFieldCfg(f: FieldDef): FieldCfg {
  ...
  return {
    ...
    restrictType: '', restrictDetail: '', maxLength: '',
    enumSelected: [], fieldHint: '',
  };
}
...
const Badge: React.FC<{ text: string; color: 'blue' | 'amber' | 'green' | 'gray' | 'red' | 'purple' }> = ({ text, color }) => {
...
const Toggle: React.FC<{ checked: boolean; onChange: (v: boolean) => void; disabled?: boolean }> = ({ checked, onChange, disabled }) => (
  <button
    ...
    className={`relative w-9 h-5 rounded-full transition-colors ${disabled ? 'opacity-50 cursor-not-allowed' : ''} ${checked ? 'bg-blue-600' : 'bg-gray-300 dark:bg-gray-600'}`}
  >
    <span className={`absolute top-0.5 w-4 h-4 rounded-full bg-white shadow transition-transform ${checked ? 'left-[18px]' : 'left-0.5'}`} />
  </button>
);

const fieldTypeBadgeColor = (type: string): 'blue' | 'amber' | 'purple' | 'gray' => {
  switch (type) {
    case '数字': return 'blue';
    case '日期': return 'amber';
    case '枚举': return 'purple';
    default: return 'gray';
  }
};
...
const LicenseTypeManager: React.FC = () => {
  ...
  const handleDrop = useCallback((targetId: string) => {
    ...
    setAuthTypes(prev => {
      ...
      return sorted.map((a, i) => ({ ...a, order: i + 1 }));
    });
    setDragId(null);
    setDragOverId(null);
  }, [dragId]);
  ...
  const updateCfg = (atId: string, fKey: string, prop: keyof FieldCfg, val: FieldCfg[keyof FieldCfg]) => {
    setAuthTypes(prev => prev.map(at => {
      ...
      if (prop === 'displayMode' && val === 'hide') {
        Object.assign(fc, { condField: '', condValue: '', valueMode: 'manual', autoSource: '', allowManualEdit: false, required: false, defaultType: '', defaultValue: '', restrictType: '', restrictDetail: '', maxLength: '', enumSelected: [], fieldHint: '' });
      }
      if (prop === 'displayMode' && val !== 'conditional') { fc.condField = ''; fc.condValue = ''; }
      if (prop === 'valueMode') {
        if (val === 'manual') { fc.autoSource = ''; fc.allowManualEdit = false; }
        if (val === 'default') { fc.autoSource = ''; }
      }
  ...
  const confirmAdd = () => {
    ...
    if (!addForm.name.trim()) errors.name = '请输入授权名称';
  ...
  const confirmCopy = () => {
    ...
    if (!src) return;
    if (!copyName.trim()) { setCopyError('请输入授权名称'); return; }
    if (authTypes.some(a => a.name === copyName.trim())) { setCopyError(`授权名称「${copyName.trim()}」已存在，不允许重名`); return; }
    const maxOrder = authTypes.reduce((m, a) => Math.max(m, a.order), 0);
  ...
  const saveDetail = (id: string) => {
    ...
    FIELD_GROUPS.forEach(g => g.fields.forEach(f => {
      const fc = at.fieldCfg[f.key];
      if (fc.displayMode === 'hide') return;
      if (fc.displayMode === 'conditional' && (!fc.condField || !fc.condValue)) {
        errors.push(`「${f.name}」条件展示的字段名和条件值不能为空`);
      }
      if (fc.valueMode === 'auto' && !fc.autoSource) {
        errors.push(`「${f.name}」自动取值的来源字段不能为空`);
      }
    }));
    if (errors.length > 0) {
      alert('保存失败：\n' + errors.join('\n'));
      ...
      <div className="p-4 lg:p-6 max-w-[2400px] mx-auto animate-page-enter">
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-bold text-gray-900 dark:text-white">{detailAt.name} — 下单字段配置</h1>
          ...
          <div className="flex gap-2">
            ...
            <button onClick={() => saveDetail(detailAt.id)} className="px-3 py-1.5 bg-blue-600 text-white rounded-lg text-sm hover:bg-blue-700 flex items-center gap-1.5"><Save className="w-3.5 h-3.5" />保存</button>
          </div>
        </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-gray-50 dark:bg-black/20 text-xs text-gray-400 font-semibold uppercase">
                    <th className="px-3 py-2 text-left w-[10%]">字段名称</th>
                    <th className="px-3 py-2 text-center w-[6%]">字段类型</th>
                    ...
                    <th className="px-3 py-2 text-left w-[13%]">默认值</th>
                    <th className="px-3 py-2 text-left w-[14%]">填写限制</th>
                    <th className="px-3 py-2 text-left w-[13%]">字段说明</th>
                  </tr>
                </thead>
                <tbody>
                  {group.fields.map(f => {
                    ...
                    const isText = f.type === '文本';
                    const hasEnumDefs = f.enumDefs && f.enumDefs.length > 0;
                    return (
                      <tr key={f.key} className={`border-t border-gray-100 dark:border-white/5 ${isHidden ? 'opacity-40' : ''}`}>
                        ...
                        <td className="px-3 py-2.5 text-center">
                          <Badge text={f.type || '文本'} color={fieldTypeBadgeColor(f.type)} />
                        </td>
                        {/* 展示控制 */}
                        <td className="px-3 py-2.5">
                            ...
                            <div className="space-y-1">
                              ...
                              {fc.displayMode === 'conditional' && (
                                <div className="flex items-center gap-1 text-[10px] text-gray-500 flex-wrap">
                                  <span>当字段【</span>
                                  <input value={fc.condField} onChange={e => updateCfg(detailAt.id, f.key, 'condField', e.target.value)} className="w-16 px-1 py-0.5 border border-gray-200 dark:border-white/10 rounded text-[10px] bg-transparent" placeholder="字段名" />
                                  <span>】=</span>
                        ...
                        {/* 必填 */}
                        <td className="px-3 py-2.5 text-center">
                          <Toggle checked={fc.required} onChange={v => !disabled && updateCfg(detailAt.id, f.key, 'required', v)} disabled={disabled} />
                        </td>
                        {/* 默认值 */}
                        <td className="px-3 py-2.5">
                          {disabled ? <span className="text-xs text-gray-300">—</span> : (
                            <div className="space-y-1">
                              ...
                              {fc.defaultType === 'text' && <input value={fc.defaultValue} onChange={e => updateCfg(detailAt.id, f.key, 'defaultValue', e.target.value)} className="w-full text-[10px] px-1 py-0.5 border border-gray-200 dark:border-white/10 rounded bg-transparent" placeholder="输入默认文字" />}
                              {fc.defaultType === 'number' && <input type="number" value={fc.defaultValue} onChange={e => updateCfg(detailAt.id, f.key, 'defaultValue', e.target.value)} className="w-full text-[10px] px-1 py-0.5 border border-gray-200 dark:border-white/10 rounded bg-transparent" placeholder="输入默认数字" />}
                              {fc.defaultType === 'variable' && (
                                <>
                                  <input value={fc.defaultValue} onChange={e => updateCfg(detailAt.id, f.key, 'defaultValue', e.target.value)} className="w-full text-[10px] px-1 py-0.5 border border-gray-200 dark:border-white/10 rounded bg-transparent" placeholder={'如：授权数量为${quantity}'} />
                                  <div className="text-[9px] text-gray-400 leading-snug">{'用 ${字段名} 引用变量'}</div>
                                </>
                              )}
                            </div>
                          )}
                          ...
                          {disabled ? <span className="text-xs text-gray-300">—</span> : (
                            <div className="space-y-1">
                              <select value={fc.restrictType} onChange={e => updateCfg(detailAt.id, f.key, 'restrictType', e.target.value)} className="text-[10px] border border-gray-200 dark:border-white/10 rounded px-1 py-0.5 bg-white dark:bg-transparent text-gray-700 dark:text-gray-300 w-full">
                                ...
                                <option value="numeric">数值型限制</option>
                                {hasEnumDefs && <option value="enum">枚举值限制</option>}
                              </select>
                              {fc.restrictType === 'numeric' && <input value={fc.restrictDetail} onChange={e => updateCfg(detailAt.id, f.key, 'restrictDetail', e.target.value)} className="w-full text-[10px] px-1 py-0.5 border border-gray-200 dark:border-white/10 rounded bg-transparent" placeholder="如：≥0, 整数, ≤100" />}
                              {fc.restrictType === 'enum' && hasEnumDefs && (
                              ...
                              {isText && (
                                <div className="flex items-center gap-1 mt-1 text-[10px] text-gray-500">
                                  字数限制
                                  <input type="number" min={1} value={fc.maxLength} onChange={e => updateCfg(detailAt.id, f.key, 'maxLength', e.target.value ? Number(e.target.value) : '')} className="w-14 px-1 py-0.5 border border-gray-200 dark:border-white/10 rounded text-[10px] bg-transparent text-center" placeholder="不限" />
                                  字
                                </div>
                              )}
                            </div>
    ...
    <div className="p-4 lg:p-6 max-w-[2400px] mx-auto animate-page-enter">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-3">
          ...
          <button onClick={() => { setShowAddModal(true); setAddErrors({}); }} className="px-3 py-1.5 bg-blue-600 text-white rounded-lg text-sm hover:bg-blue-700 flex items-center gap-1.5"><Plus className="w-3.5 h-3.5" />新增授权</button>
        </div>
          ...
          <tbody>
            ...
            {filtered.map(at => (
              <tr
                key={at.id}
                draggable
                onDragStart={() => handleDragStart(at.id)}
                onDragOver={e => handleDragOver(e, at.id)}
                onDrop={() => handleDrop(at.id)}
                onDragEnd={handleDragEnd}
                onClick={() => setDetailId(at.id)}
                className={`border-t border-gray-100 dark:border-white/5 cursor-pointer hover:bg-blue-50/30 dark:hover:bg-white/5 transition ${dragId === at.id ? 'opacity-30' : ''} ${dragOverId === at.id && dragId !== at.id ? 'ring-2 ring-blue-400 ring-inset bg-blue-50/50 dark:bg-blue-900/20' : ''}`}
              >
                <td className="px-2 py-3 text-center">
                  <div className="flex items-center justify-center cursor-grab active:cursor-grabbing" onClick={e => e.stopPropagation()}>
                    <GripVertical className="w-4 h-4 text-gray-300 dark:text-gray-600 hover:text-gray-500 dark:hover:text-gray-400 transition-colors" />
                  </div>
                </td>
        ...
        <ModalPortal>
          <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50" onClick={() => setShowCopyModal(false)}>
            <div className="bg-white dark:bg-[#2C2C2E] rounded-2xl p-6 w-[420px] shadow-2xl" onClick={e => e.stopPropagation()}>
              ...
              <div>
                <label className="text-xs font-bold text-gray-500 mb-1 block">新授权名称</label>
                <input value={copyName} onChange={e => setCopyName(e.target.value)} className="w-full px-3 py-2 border border-gray-200 dark:border-white/10 rounded-lg text-sm bg-transparent text-gray-800 dark:text-white" />
                {copyError && <div className="text-xs text-red-500 mt-1">{copyError}</div>}
              </div>
              <div className="flex justify-end gap-2 mt-5">
