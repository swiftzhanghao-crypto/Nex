...
const UserManager: React.FC<UserManagerProps> = ({ users, setUsers, departments, roles, setRoles, channels, defaultTab = 'USERS' }) => {
  ...
  const permissionTree = [
      ...
      },
      {
          ...
          subgroups: [
              {
                  id: 's_product_display', label: '产品展示',
                  permissions: [
                      { id: 'product_display_view',    label: '查看产品展示', desc: '访问产品展示页面，浏览产品目录' },
  ...
  type PermSubgroup = { id: string; label: string; dependsOn?: string; permissions: { id: string; label: string; desc: string }[] };
  type PermGroup = { id: string; label: string; subgroups: PermSubgroup[] };

  // 从树中收集所有权限点的扁平列表，供原有 toggle 逻辑复用
  const permissionModules = permissionTree.map(g => ({
      id: g.id,
      label: g.label,
  ...
  return (
    <div className="p-6 lg:p-8 max-w-[1600px] mx-auto space-y-6 animate-page-enter relative h-[calc(100vh-64px)] flex flex-col">
      ...
      {/* --- USERS TAB --- */}
      {activeTab === 'USERS' && (
          <div className="unified-card dark:bg-[#1C1C1E] -gray-100 dark:-white/10 flex flex-col animate-fade-in flex-1">
      ...
      {activeTab === 'ROLES' && (
          <div className="flex gap-6 flex-1 min-h-0 animate-fade-in">
              {/* Role List */}
              <div className="unified-card w-1/4 min-w-[250px] dark:bg-[#1C1C1E] -gray-100 dark:-white/10 flex flex-col">
              ...
              {/* Role Config */}
              <div className="unified-card flex-1 dark:bg-[#1C1C1E] -gray-100 dark:-white/10 flex flex-col">
                  {selectedRoleId ? (
                      isEditingRole ? (
                          <>
