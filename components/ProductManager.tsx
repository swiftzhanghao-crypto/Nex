...
import { Product, ProductSku, InstallPackage, AtomicCapability, CapabilityType, ProductRightDefinition, RightPackage, RightValue, RightDataType, LicenseTypeDefinition, LicenseType, LicenseUnit, User, RoleDefinition } from '../types';
...
const tabPermissionMap: Record<string, string> = {
  SPU: 'product_tab_spu',
  SKU: 'product_tab_sku',
  LicenseTypes: 'product_tab_license',
  Rights: 'product_tab_rights',
  Packages: 'product_tab_packages',
  Atomic: 'product_tab_atomic',
};
...
const ProductManager: React.FC<ProductManagerProps> = ({ 
  ...
  const allTabs = ['SPU', 'SKU', 'LicenseTypes', 'Rights', 'Packages', 'Atomic'] as const;
  const visibleTabs = allTabs.filter(t => hasPermission(tabPermissionMap[t]));

  const [activeTab, setActiveTab] = useState<'SPU' | 'SKU' | 'Rights' | 'Packages' | 'Atomic' | 'LicenseTypes'>(() =>
  ...
  return (
    <div className="p-6 lg:p-8 max-w-[1600px] mx-auto space-y-6 animate-fade-in relative">
      <div className="flex justify-between items-center gap-4">
        <div className="flex bg-gray-100 dark:bg-white/10 p-1 rounded-lg overflow-x-auto">
            {visibleTabs.map((tab) => (
                <button 
                    key={tab}
                    onClick={() => setActiveTab(tab as typeof activeTab)}
                    className={`px-4 py-2 rounded-lg text-sm font-medium transition whitespace-nowrap ${activeTab === tab ? 'bg-white dark:bg-[#1C1C1E] shadow text-gray-900 dark:text-white' : 'text-gray-500 hover:text-gray-900 dark:text-gray-400 dark:hover:text-white'}`}
                >
                    {tab === 'SPU' ? '产品列表' : 
                     tab === 'SKU' ? '规格列表' : 
      ...
      <div className="space-y-6 animate-fade-in">
          {(activeTab !== 'Atomic' && activeTab !== 'Rights' && activeTab !== 'Packages' && activeTab !== 'LicenseTypes') && (
              <div className="flex justify-between items-center">
                  <div className="unified-card dark:bg-[#1C1C1E] p-2.5 -gray-100 dark:-white/10 flex items-center gap-3 w-80">
                    ...
                    {(pageItems as Product[]).map(product => (
                        ...
                        <td className="p-4">
                            <div className="flex flex-wrap gap-1">
                                {product.tags?.map(t => (
                                    <span key={t} className={`text-[10px] px-2 py-0.5 rounded-lg border font-bold ${getTagColor(t)}`}>
                                        {t}
                                    </span>
                                ))}
                            </div>
                    ...
                    <div className="flex justify-between items-center p-4 border-t border-gray-100 dark:border-white/10 bg-gray-50 dark:bg-white/5">
                        ...
                        <div className="flex items-center gap-4">
                            <button 
                                ...
                                className="px-3 py-1 rounded-lg bg-white dark:bg-black border border-gray-200 dark:border-white/10 text-sm disabled:opacity-50 hover:bg-gray-50 dark:hover:bg-white/5 transition"
                            >
                                上一页
                            </button>
                      ...
                      <tbody className="divide-y divide-gray-100 dark:divide-white/5 text-base">
                          {(pageItems as (ProductSku & { parentName: string; parentId: string })[]).map(sku => (
                              <tr key={sku.id} className="group hover:bg-gray-100/80 dark:hover:bg-white/[0.08] even:bg-gray-50/50 dark:even:bg-white/[0.02] transition-colors">
                                  ...
                                  <td className="p-4">
                                      <span className="bg-gray-100 dark:bg-white/10 px-2 py-1 rounded-lg text-xs text-gray-600 dark:text-gray-300">
              ...
              <div className="space-y-6">
                  <div className="flex justify-between items-center">
                      ...
                      <button 
                        ...
                        className="bg-black dark:bg-white text-white dark:text-black px-4 py-2 rounded-lg flex items-center gap-2 hover:opacity-80 transition shadow-apple text-sm font-medium"
                        >
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                      {licenseDefs.map(def => (
                          <div key={def.id} className="unified-card dark:bg-[#1C1C1E] p-4 -gray-100 dark:-white/10 hover: transition">
                              <div className="flex justify-between items-start mb-2">
                                  ...
                                  <span className="text-[10px] bg-gray-50 dark:bg-white/10 text-gray-500 px-2 py-0.5 rounded-lg font-mono">{def.code}</span>
                              </div>
              ...
              <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                      {productRights.map(right => (
                          <div key={right.id} className="unified-card dark:bg-[#1C1C1E] p-4 -gray-100 dark:-white/10 hover: transition">
                              <div className="flex justify-between items-start mb-2">
                                  <h4 className="font-bold text-gray-900 dark:text-white">{right.name}</h4>
                                  <span className="text-xs bg-gray-50 dark:bg-white/10 text-gray-500 px-2 py-0.5 rounded-lg font-mono">{right.code}</span>
                              </div>
              ...
              <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      {rightPackages.map(pkg => (
                          <div key={pkg.id} className="unified-card dark:bg-[#1C1C1E] p-5 -gray-100 dark:-white/10 hover: transition">
              ...
              <div className="space-y-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                      {atomicCapabilities.map(atom => (
                          <div key={atom.id} className="unified-card dark:bg-[#1C1C1E] p-4 -gray-100 dark:-white/10 hover: transition group relative">
                              <div className="flex justify-between items-start mb-2">
                                  <div className={`p-2 rounded-lg ${atom.type === 'Component' ? 'bg-purple-100 text-purple-600' : atom.type === 'Service' ? 'bg-blue-100 text-blue-600' : 'bg-orange-100 text-orange-600'}`}>
                              ...
                              <div className="flex justify-between items-center text-xs">
                                  ...
                                  <span className="bg-gray-50 dark:bg-white/10 text-gray-600 dark:text-gray-300 px-2 py-0.5 rounded-lg">{atom.type}</span>
                              </div>
                          </div>
                      ))}
                  </div>
      ...
      {isModalOpen && (
        ...
        <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-[500] animate-fade-in p-4">
          <div className="unified-card dark:bg-[#1C1C1E] shadow-2xl w-full max-w-3xl m-4 flex flex-col max-h-[90vh] animate-modal-enter -white/10">
            ...
            <div className="p-6 overflow-y-auto space-y-6 flex-1">
              ...
              <div>
                  ...
                  <select 
                      ...
                      className="w-full border border-gray-300 dark:border-white/10 rounded-lg p-2.5 bg-white dark:bg-black text-gray-900 dark:text-white outline-none"
                  >
      ...
      {isLicenseModalOpen && (
          ...
          <ModalPortal>
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-[500] animate-fade-in p-4">
              <div className="unified-card dark:bg-[#1C1C1E] shadow-2xl w-full max-w-md p-6 -white/10">
                  <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-4">新增授权类型</h3>
                  <div className="space-y-4">
                      <div><label className="block text-xs font-medium text-gray-500 mb-1">名称 (Name)</label><input value={licenseForm.name} onChange={e => setLicenseForm({...licenseForm, name: e.target.value})} className="w-full border rounded-lg p-2 dark:bg-black dark:border-white/10 dark:text-white" placeholder="如：年度订阅" /></div>
                      <div><label className="block text-xs font-medium text-gray-500 mb-1">代码 (Code)</label><input value={licenseForm.code} onChange={e => setLicenseForm({...licenseForm, code: e.target.value})} className="w-full border rounded-lg p-2 dark:bg-black dark:border-white/10 dark:text-white font-mono" placeholder="SUB-YEAR" /></div>
                      
                      <div className="grid grid-cols-2 gap-3">
                          <div>
                              <label className="block text-xs font-medium text-gray-500 mb-1">类型</label>
                              <select value={licenseForm.type} onChange={e => setLicenseForm({...licenseForm, type: e.target.value as LicenseType})} className="w-full border rounded-lg p-2 dark:bg-black dark:border-white/10 dark:text-white">
                          ...
                          <div>
                              ...
                              <select value={licenseForm.periodUnit} onChange={e => setLicenseForm({...licenseForm, periodUnit: e.target.value as LicenseUnit})} className="w-full border rounded-lg p-2 dark:bg-black dark:border-white/10 dark:text-white">
                      ...
                      <div className="grid grid-cols-2 gap-3">
                          <div><label className="block text-xs font-medium text-gray-500 mb-1">期限值</label><input type="number" value={licenseForm.period} onChange={e => setLicenseForm({...licenseForm, period: Number(e.target.value)})} className="w-full border rounded-lg p-2 dark:bg-black dark:border-white/10 dark:text-white" /></div>
                          <div><label className="block text-xs font-medium text-gray-500 mb-1">适用范围</label><input value={licenseForm.scope} onChange={e => setLicenseForm({...licenseForm, scope: e.target.value})} className="w-full border rounded-lg p-2 dark:bg-black dark:border-white/10 dark:text-white" placeholder="如: 1 User" /></div>
                      </div>

                      <div><label className="block text-xs font-medium text-gray-500 mb-1">描述</label><textarea value={licenseForm.description} onChange={e => setLicenseForm({...licenseForm, description: e.target.value})} className="w-full border rounded-lg p-2 resize-none dark:bg-black dark:border-white/10 dark:text-white" rows={2}/></div>
                  </div>
                  <div className="flex justify-end gap-2 mt-6">
                      <button onClick={() => setIsLicenseModalOpen(false)} className="px-4 py-2 text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-white/10 rounded-lg">取消</button>
                      <button onClick={handleSaveLicenseType} className="px-4 py-2 bg-black dark:bg-white text-white dark:text-black rounded-lg">保存</button>
                  </div>
              </div>
          </div>
          </ModalPortal>
              ...
              <div className="unified-card dark:bg-[#1C1C1E] shadow-2xl w-full max-w-md p-6 -white/10">
                  ...
                  <div className="space-y-4">
                      <div>
                          ...
                          <input value={atomForm.name} onChange={e => setAtomForm({...atomForm, name: e.target.value})} className="w-full border rounded-lg p-2 dark:bg-black dark:border-white/10 dark:text-white" />
                      </div>
                      <div>
                          <label className="block text-xs font-medium text-gray-500 mb-1">类型</label>
                          <select value={atomForm.type} onChange={e => setAtomForm({...atomForm, type: e.target.value as CapabilityType})} className="w-full border rounded-lg p-2 dark:bg-black dark:border-white/10 dark:text-white">
                      ...
                      <div>
                          ...
                          <textarea value={atomForm.description} onChange={e => setAtomForm({...atomForm, description: e.target.value})} className="w-full border rounded-lg p-2 resize-none dark:bg-black dark:border-white/10 dark:text-white" rows={3}/>
                      </div>
                  </div>
                  <div className="flex justify-end gap-2 mt-6">
                      <button onClick={() => setIsAtomModalOpen(false)} className="px-4 py-2 text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-white/10 rounded-lg">取消</button>
                      <button onClick={handleSaveAtom} className="px-4 py-2 bg-black dark:bg-white text-white dark:text-black rounded-lg">保存</button>
                  </div>
              </div>
          </div>
          </ModalPortal>
      ...
      {isRightsModalOpen && (
          ...
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-[500] animate-fade-in p-4">
              <div className="unified-card dark:bg-[#1C1C1E] shadow-2xl w-full max-w-md p-6 -white/10">
                  ...
                  <div className="space-y-4">
                      <div><label className="block text-xs font-medium text-gray-500 mb-1">权益名称</label><input value={rightsForm.name} onChange={e => setRightsForm({...rightsForm, name: e.target.value})} className="w-full border rounded-lg p-2 dark:bg-black dark:border-white/10 dark:text-white" placeholder="如：云空间" /></div>
                      <div><label className="block text-xs font-medium text-gray-500 mb-1">权益代码 (Code)</label><input value={rightsForm.code} onChange={e => setRightsForm({...rightsForm, code: e.target.value})} className="w-full border rounded-lg p-2 dark:bg-black dark:border-white/10 dark:text-white font-mono" placeholder="storage_quota" /></div>
                      <div>
                          <label className="block text-xs font-medium text-gray-500 mb-1">数据类型</label>
                          <select value={rightsForm.dataType} onChange={e => setRightsForm({...rightsForm, dataType: e.target.value as RightDataType})} className="w-full border rounded-lg p-2 dark:bg-black dark:border-white/10 dark:text-white">
                              <option value="Number">数字 (Number)</option>
                              <option value="Boolean">布尔 (Boolean)</option>
                              <option value="Text">文本 (Text)</option>
                          </select>
                      </div>
                      <div><label className="block text-xs font-medium text-gray-500 mb-1">单位</label><input value={rightsForm.unit} onChange={e => setRightsForm({...rightsForm, unit: e.target.value})} className="w-full border rounded-lg p-2 dark:bg-black dark:border-white/10 dark:text-white" placeholder="如：GB, 个" /></div>
                      <div><label className="block text-xs font-medium text-gray-500 mb-1">描述</label><textarea value={rightsForm.description} onChange={e => setRightsForm({...rightsForm, description: e.target.value})} className="w-full border rounded-lg p-2 resize-none dark:bg-black dark:border-white/10 dark:text-white" rows={2}/></div>
                  </div>
                  <div className="flex justify-end gap-2 mt-6">
                      <button onClick={() => setIsRightsModalOpen(false)} className="px-4 py-2 text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-white/10 rounded-lg">取消</button>
                      <button onClick={handleSaveRights} className="px-4 py-2 bg-black dark:bg-white text-white dark:text-black rounded-lg">保存</button>
                  </div>
              </div>
          </div>
          </ModalPortal>
      ...
      {isPackageModalOpen && (
          ...
          <ModalPortal>
          <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-[500] animate-fade-in p-4">
              <div className="unified-card dark:bg-[#1C1C1E] shadow-2xl w-full max-w-2xl p-6 -white/10 max-h-[90vh] flex flex-col">
                  <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-4">新增权益套餐</h3>
                  <div className="space-y-4 overflow-y-auto flex-1">
                      <div><label className="block text-xs font-medium text-gray-500 mb-1">套餐名称</label><input value={pkgForm.name} onChange={e => setPkgForm({...pkgForm, name: e.target.value})} className="w-full border rounded-lg p-2 dark:bg-black dark:border-white/10 dark:text-white" /></div>
                      <div><label className="block text-xs font-medium text-gray-500 mb-1">描述</label><textarea value={pkgForm.description} onChange={e => setPkgForm({...pkgForm, description: e.target.value})} className="w-full border rounded-lg p-2 resize-none dark:bg-black dark:border-white/10 dark:text-white" rows={2}/></div>
                              ...
                              {productRights.map(def => {
                                  ...
                                  return (
                                      <div key={def.id} className={`flex items-center gap-3 p-3 rounded-lg border transition ${selected ? 'bg-blue-50 border-blue-200 dark:bg-blue-900/20 dark:border-blue-800' : 'border-gray-200 dark:border-white/10'}`}>
                                          <input type="checkbox" checked={!!selected} onChange={() => handleTogglePkgRight(def)} className="w-4 h-4 rounded-lg" />
                                              ...
                                              <div className="flex items-center gap-2">
                                                  {def.dataType === 'Boolean' ? (
                                                      <select 
                                                        ...
                                                        className="border rounded-lg p-1 text-sm bg-white dark:bg-black dark:text-white dark:border-white/10"
                                                      >
                                                      ...
                                                      <input 
                                                        ...
                                                        className="border rounded-lg p-1 w-24 text-sm bg-white dark:bg-black dark:text-white dark:border-white/10"
                                                      />
                                                  )}
                  ...
                  <div className="flex justify-end gap-2 mt-6 pt-4 border-t dark:border-white/10">
                      <button onClick={() => setIsPackageModalOpen(false)} className="px-4 py-2 text-gray-600 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-white/10 rounded-lg">取消</button>
                      <button onClick={handleSavePackage} className="px-4 py-2 bg-black dark:bg-white text-white dark:text-black rounded-lg">保存套餐</button>
                  </div>
              </div>
          </div>
          </ModalPortal>
