...
import { LayoutDashboard, ShoppingCart, Package, Users, Menu, X, ChevronDown, Shield, Building2, Network, Target, Moon, Sun, Settings, Activity, Maximize, Minimize, PanelLeftClose, PanelLeftOpen, Layers, BarChart3, PieChart, Contact2, Zap, FileText, ArrowUpCircle, Database, Link as LinkIcon, Settings2, Monitor, LayoutList, BookOpen, FileBadge, Banknote, Receipt, TrendingUp, KeyRound, PackageCheck } from 'lucide-react';
...
const Layout: React.FC<LayoutProps> = ({ children, currentUser, users, setCurrentUser, roles }) => {
  ...
  const topNavItems = [
      ...
      { id: 'PRODUCT_CENTER', label: '产品中心', path: '/product-center', permissions: ['product_display_view', 'product_display_preview', 'product_view', 'merchandise_view'] },
                    ...
                    {activeTopNav === 'PRODUCT_CENTER' && (
                      <>
                        {renderSectionGroup('product_main', '产品中心',
                          <>
                            <NavItem to="/product-center" icon={Layers} label="产品展示" alsoMatch={['/catalog']} permission="product_display_view" />
                            <NavItem to="/products" icon={Package} label="产品列表" permission="product_view" />
                          </>,
                          ['product_display_view', 'product_display_preview', 'product_view', 'merchandise_view']
                        )}
                      </>
                    )}
