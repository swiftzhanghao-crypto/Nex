              ...
              {configTab === 'ROW' && (
                <div className="animate-fade-in" style={{ height: 'calc(100vh - 240px)', minHeight: '420px' }}>
                  {space.resourceConfig.length === 0 ? (
                    <div className="text-center py-10 text-gray-400 text-sm border border-dashed border-gray-200 dark:border-white/10 rounded-lg">该空间未配置行权限资源</div>
                  ) : (
              ...
              {configTab === 'COLUMN' && (
                <div className="animate-fade-in">
                  {space.columnConfig.length === 0 ? (
                    <div className="text-center py-10 text-gray-400 text-sm border border-dashed border-gray-200 dark:border-white/10 rounded-lg">该空间未配置列权限</div>
                  ) : (
                                ...
                                <div key={rule.id} className="flex items-center gap-3 px-4 py-2.5">
                                  ...
                                  <div className="flex flex-wrap gap-1.5">
                                    {rule.values.length > 0 ? rule.values.map(v => (
                                      <span key={v} className="px-2 py-0.5 rounded-full bg-amber-50 dark:bg-amber-900/20 text-amber-700 dark:text-amber-300 text-xs font-medium border border-amber-100 dark:border-amber-800/30">{v}</span>
                                    )) : (
